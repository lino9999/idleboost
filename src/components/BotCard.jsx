...
import { CalendarDays, ExternalLink, Power, Trash2, Trophy } from 'lucide-react';
import Tip from './Tip';
...
export default function BotCard({ name, bot, status, standby, ownedGames, dbAvatar, dbWallet, dbStats, isStorage, activeFull, onChanged, toast }) {
  ...
  return (
    <div className="card flex flex-col p-4 transition-colors hover:border-white/[0.12]">
      ...
      {dbStats && (Number(dbStats.achievements_total) > 0 || Number(dbStats.account_created) > 0) && (
        <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
          {Number(dbStats.achievements_total) > 0 && (
            <Tip tip="CS2 achievements unlocked / total (collected by the AutoBan Checker)" block>
              <div className="flex items-center gap-1.5 rounded-lg bg-night-800/70 px-2.5 py-2">
                ...
                <div className="min-w-0">
                  <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">CS2 Ach.</div>
                  <div className="font-bold text-slate-200">
                    {Number(dbStats.achievements_unlocked) || 0}/{Number(dbStats.achievements_total)}
                  </div>
                </div>
              </div>
            </Tip>
          )}
          {Number(dbStats.account_created) > 0 && (
